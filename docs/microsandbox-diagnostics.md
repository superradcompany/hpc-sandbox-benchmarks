...
| --- | --- | --- |
| `openclaw-v2-all-throttled-fd16384-v1` | All eight V2 tasks, sequentially | Exact Node24.16.0, supported throttled commands, explicit guest soft/hard nofile16384; observed limits must match |
...
The fd16384 configuration retains every source file and task. The pinned task-boundary test opens9442 source files concurrently, independently of Vitest worker count; the unchanged helper fails at4096 and passes at16384 locally. The create manifest records requested limits, and environment.log records actual guest limits. Cloud validation is still required. The full test-types graph also failed the Go2GiB probe with an OOM kill below its time limit, so this diagnostic infrastructure does not establish that all eight tasks fit8GiB. Scored V2 remains inactive.

Each new task-outcome row retains `task` and `exitCode` and adds `started`, `elapsedSeconds`, `remainingSequenceSeconds`, and `reason`. An exhausted sequence that never invokes a task records `started=false`, zero elapsed seconds, exit124, and `reason=sequence_budget_exhausted`. A started command records `reason=command_exit`; exit124 alone cannot distinguish its inner task timeout from the outer remaining-sequence timeout. Generated unstarted rows are not evidence that a task was tested. The fixed50-minute sequence budget is unchanged; any unstarted tasks require a separately bounded follow-up before claiming complete validation.
